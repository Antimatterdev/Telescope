...
class TwitchMonitor {
    ...
    static refresh(reason) {
        ...
        if (this._lastUserRefresh === null || now.diff(moment(this._lastUserRefresh), 'minutes') >= 10) {
            ...
            TwitchApi.fetchUsers(this.channelNames)
              ...
              .catch((err) => {
                  logging.error('[TwitchMonitor]', 'Error in users refresh:', err);
              })
              .then(() => {
        ...
        if (this._pendingGameRefresh) {
            TwitchApi.fetchGames(this._watchingGameIds)
              ...
              .catch((err) => {
                  logging.error('[TwitchMonitor]', 'Error in games refresh:', err);
              })
              .then(() => {
                  if (this._pendingGameRefresh) {
                      this._pendingGameRefresh = false;
        ...
        if (!this._pendingUserRefresh && !this._pendingGameRefresh) {
            TwitchApi.fetchStreams(this.channelNames)
              ...
              .catch((err) => {
                  logging.error('[TwitchMonitor]', 'Error in streams refresh:', err);
              });
        }
    }
    ...
    static handleUserList(users) {
        ...
        if (namesSeen.length) {
            logging.twitch('[TwitchMonitor]', `Updated user info: ${namesSeen.join(', ')}`);
        }
    ...
    static handleGameList(games) {
        ...
        if (gotGameNames.length) {
            logging.twitch('[TwitchMonitor]', `Updated game info: ${gotGameNames.join(', ')}`);
        }
    ...
    static handleStreamList(streams) {
        ...
        for (let i = 0; i < nextOnlineList.length; i++) {
            ...
            if (this.activeStreams.indexOf(_chanName) === -1) {
                ...
                logging.twitch('[TwitchMonitor]', `Stream channel has gone online: ${_chanName}`);
                anyChanges = true;
            }
        ...
        for (let i = 0; i < this.activeStreams.length; i++) {
            ...
            if (nextOnlineList.indexOf(_chanName) === -1) {
                ...
                logging.twitch('[TwitchMonitor]', `Stream channel has gone offline: ${_chanName}`);
        ...
        if (!notifyFailed) {
            ...
            logging.error('[TwitchMonitor]', 'Could not notify channel, will try again next update.');
        }
