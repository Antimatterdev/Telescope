...
class TwitchApi {
  ...
  static handleApiError(err) {
    ...
    if (res.data && res.data.message) {
      logging.error('[TwitchApi]', 'API request failed with Helix error:', res.data.message, `(${res.data.error}/${res.data.status})`);
    } else {
      logging.error('[TwitchApi]', 'API request failed with error:', err.message || err);
    }
  }
